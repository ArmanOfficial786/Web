...
import {
  LoanReScheduleFormValues,
  LoanReScheduleResponseExtended,
} from "@/app/(home)/(sidebar)/Loan/OtherReports/LoanReScheduleReport/page";
...
interface LoanReScheduleFormProps {
  control: Control<LoanReScheduleFormValues>;
  handleSubmit: UseFormHandleSubmit<LoanReScheduleFormValues>;
  onSubmit: SubmitHandler<LoanReScheduleFormValues>;
  setValue: UseFormSetValue<LoanReScheduleFormValues>;
  reset: UseFormReset<LoanReScheduleFormValues>;
  reportState: LoanReScheduleResponseExtended;
  onPageChange: (page: number) => void;
  onDownload: (format: ReportFormat) => void | Promise<void>;
}

function LoanReScheduleForm({
  control,
  handleSubmit,
  onSubmit,
  setValue,
    ...
    <>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <Paper variant="outlined" sx={{ px: 2, py: 1 }}>
          ...
          >
            Loan Re-Schedule Report
          </Typography>
          <Divider sx={{ mb: 0.5 }} />

          {/* ── From/To Date ─────────────────────────────────────────────── */}
          <Box sx={{ mb: 0.5 }}>
            <DateFields<LoanReScheduleFormValues>
              control={control}
          ...
          >
            <OfficeNameField<LoanReScheduleFormValues>
              control={control}
              branchFieldName="branchIds"
              multiple={false}
            />
            <SoleSelectGroupField<LoanReScheduleFormValues>
              control={control}
              setValue={setValue}
              branchFieldName="branchIds"
          ...
          >
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Box
                ...
                width="100%"
              >
                <ViewReportButton<LoanReScheduleFormValues>
                  control={control}
...
export default React.memo(LoanReScheduleForm);
