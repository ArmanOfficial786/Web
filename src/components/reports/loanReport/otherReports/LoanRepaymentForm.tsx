// components/reports/loanReport/otherReports/LoanRepaymentForm.tsx
"use client";
...
import type {
  ...
  LoanRepaymentResponseExtended,
} from "@/app/(home)/(sidebar)/Loan/OtherReports/LoanRepaymentReport/page";
...
interface LoanRepaymentFormProps {
  control: Control<LoanRepaymentFormValues>;
  handleSubmit: UseFormHandleSubmit<LoanRepaymentFormValues>;
  onSubmit: SubmitHandler<LoanRepaymentFormValues>;
  setValue: UseFormSetValue<LoanRepaymentFormValues>;
  reset: UseFormReset<LoanRepaymentFormValues>;
  reportState: LoanRepaymentResponseExtended;
...
function LoanRepaymentForm({
  ...
  const memberLookupConfig = React.useMemo(
    () => MemberLookupConfig<LoanRepaymentFormValues>(),
    [],
  );
    ...
    <>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <Paper variant="outlined" sx={{ px: 2, py: 1 }}>
          ...
          {/* ── Office Name ───────────────────────────────────────────────── */}
          <Box sx={{ mb: 0.5 }}>
            <OfficeNameField<LoanRepaymentFormValues>
              control={control}
          ...
          <Grid container spacing={1} alignItems="center">
            <Grid size={{ xs: 12, md: 6 }}>
              <Box
                ...
                width="100%"
              >
                <ViewReportButton<LoanRepaymentFormValues>
                  control={control}
...
export default React.memo(LoanRepaymentForm);
