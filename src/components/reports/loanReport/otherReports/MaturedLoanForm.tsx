...
import {
  MaturedLoanFormValues,
  MaturedLoanResponseExtended,
} from "@/app/(home)/(sidebar)/Loan/OtherReports/MaturedLoanReport/page";
...
interface MaturedLoanFormProps {
  control: Control<MaturedLoanFormValues>;
  handleSubmit: UseFormHandleSubmit<MaturedLoanFormValues>;
  onSubmit: SubmitHandler<MaturedLoanFormValues>;
  setValue: UseFormSetValue<MaturedLoanFormValues>;
  reset: UseFormReset<MaturedLoanFormValues>;
  reportState: MaturedLoanResponseExtended;
  onPageChange: (page: number) => void;
  onDownload: (format: ReportFormat) => void | Promise<void>;
}

function MaturedLoanForm({
  control,
  handleSubmit,
  onSubmit,
  setValue,
  ...
  const memberLookupConfig = React.useMemo(
    () => MemberLookupConfig<MaturedLoanFormValues>(),
    [],
  );
    ...
    <>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <Paper variant="outlined" sx={{ px: 2, py: 1 }}>
          ...
          {/* ── From/To Date ─────────────────────────────────────────────── */}
          <Box sx={{ mb: 0.5 }}>
            <DateFields<MaturedLoanFormValues>
              control={control}
          ...
          >
            <OfficeNameField<MaturedLoanFormValues>
              control={control}
              branchFieldName="branchIds"
              multiple={false}
            />
            <SoleSelectGroupField<MaturedLoanFormValues>
              control={control}
              setValue={setValue}
              branchFieldName="branchIds"
          ...
          >
            <OrderByField<MaturedLoanFormValues>
              control={control}
              name="orderBy"
              reportKey="matured-loan-report"
            />
            <Grid size={{ xs: 12, md: 6 }}>
              <Box
                ...
                width="100%"
              >
                <ViewReportButton<MaturedLoanFormValues>
                  control={control}
...
export default React.memo(MaturedLoanForm);
