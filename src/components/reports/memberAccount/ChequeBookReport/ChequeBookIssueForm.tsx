// components/reports/memberAccount/OthersReport/ChequeBookIssueForm.tsx
"use client";
...
import {
  ChequeBookIssueFormValues,
  ChequeBookIssueResponseExtended,
} from "@/app/(home)/(sidebar)/MemberAc/ChequeBookReport/ChequeBookIssueReport/page";

export type { ReportFormat };

interface ChequeBookIssueFormProps {
  control: Control<ChequeBookIssueFormValues>;
  handleSubmit: UseFormHandleSubmit<ChequeBookIssueFormValues>;
  onSubmit: SubmitHandler<ChequeBookIssueFormValues>;
  setValue: UseFormSetValue<ChequeBookIssueFormValues>;
  reset: UseFormReset<ChequeBookIssueFormValues>;
  reportState: ChequeBookIssueResponseExtended;
  onPageChange: (page: number) => void;
  onDownload: (format: ReportFormat) => void | Promise<void>;
}

function ChequeBookIssueForm({
  control,
  handleSubmit,
  onSubmit,
  setValue,
  ...
  const memberLookupConfig = useMemo(
    () => MemberLookupConfig<ChequeBookIssueFormValues>(),
    [],
  );
    ...
    <>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <Paper variant="outlined" sx={{ px: 2, py: 1 }}>
          ...
          {/* ── From/To Date ─────────────────────────────────────────────── */}
          <Box sx={{ mb: 0.5 }}>
            <DateFields<ChequeBookIssueFormValues>
              control={control}
          ...
          >
            <OfficeNameField<ChequeBookIssueFormValues>
              control={control}
              branchFieldName="branchIds"
            />
            <OrderByField<ChequeBookIssueFormValues>
              control={control}
              name="orderBy"
              reportKey="cheque-book-issue-report" 
            />
          </Box>
          <Divider sx={{ mb: 0.5 }} />
          ...
          <Grid container spacing={1} alignItems="center">
            <Grid size={{ xs: 12, md: 6 }}>
              <Box
                ...
                width="100%"
              >
                <ViewReportButton<ChequeBookIssueFormValues>
                  control={control}
...
export default React.memo(ChequeBookIssueForm);
