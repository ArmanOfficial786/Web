...
// ── DisplayType options — Horizontal / Vertical ──────────────────────────────
const displayTypeOptions = [
  { value: "Horizontal", label: "Horizontal" },
  { value: "Vertical", label: "Vertical" },
];

interface PLAccountFormProps {
  control: Control<PLAccountFormValues>;
  handleSubmit: UseFormHandleSubmit<PLAccountFormValues>;
  onSubmit: SubmitHandler<PLAccountFormValues>;
  setValue: UseFormSetValue<PLAccountFormValues>;
  reset: UseFormReset<PLAccountFormValues>;
  reportState: PLAccountResponseExtended;
  onPageChange: (page: number) => void;
  onDownload: (format: ReportFormat) => void | Promise<void>;
}

function PLAccountForm({
  control,
  handleSubmit,
  onSubmit,
  setValue,
    ...
    <>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <Paper variant="outlined" sx={{ px: 2, py: 1 }}>
          ...
          >
            <OfficeNameField<PLAccountFormValues>
              control={control}
          ...
          >
            <FieldRow label="Display Type">
              <RadioInput
                name="displayType"
                control={control}
                radioOptions={displayTypeOptions}
                row
              />
            </FieldRow>

            <OrderByField<PLAccountFormValues>
              control={control}
              name="orderBy"
              reportKey="pl-account-report"
            />
          </Box>
          <Divider sx={{ mb: 0.5 }} />

          {/* ── Same Company + Nepali Report ─────────────────────────────── */}
          <Box
            sx={{
              display: "grid",
              gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" },
          ...
          >
            <SameCompanyField<PLAccountFormValues>
              control={control}
              labelPlacement="end"
            />
            <NepaliReportField<PLAccountFormValues>
              control={control}
              labelPlacement="end"
            />
          </Box>
          ...
          <Grid container spacing={1} alignItems="center">
            <Grid size={{ xs: 12, md: 6 }}>
              <Box
                ...
                width="100%"
              >
                <ViewReportButton<PLAccountFormValues>
                  control={control}
...
export default React.memo(PLAccountForm);
