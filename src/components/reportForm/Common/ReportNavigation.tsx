...
//   pdfData?: string; // base64 PDF data for print/download. If undefined, print/download is disabled.
//   currentPage: number;
//   totalPages: number;
...
interface ReportNavigationProps {
  pdfData?: string; // now optional – undefined means HTML view
  currentPage: number;
  totalPages: number;
  onPageChange: (page: number) => void;
  onDownload: (format: ReportFormat) => void | Promise<void>;
...
const ReportNavigation: React.FC<ReportNavigationProps> = ({
  ...
  // ── Print (only when pdfData exists) ───────────────────────────────────
  const handlePrint = () => {
    if (!pdfData) {
      // Should never be called because the button is hidden
      return;
    }
    try {
      const win = window.open("", "_blank");
  ...
  return (
    ...
    >
      ...
      {/* Print + Download */}
      <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
        {/* ── Print button – only shown when pdfData exists ────────────── */}
        {pdfData && (
          <Tooltip title={t("print")}>
            <span>
              <Button
