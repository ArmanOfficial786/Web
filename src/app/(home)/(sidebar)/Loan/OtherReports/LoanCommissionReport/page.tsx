...
import LoanCommissionForm from "@/components/reports/loanReport/otherReports/LoanCommissionForm";
...
const schema: yup.ObjectSchema<LoanCommissionFormValues> = yup
  .object({
    fromDateBs: yup
      .string()
      .nullable()
...
export default function LoanCommissionPage() {
  ...
  const [lastRequest, setLastRequest] =
    useState<LoanCommissionRequestDto | null>(null);

  const { control, handleSubmit, setValue, reset } =
    useForm<LoanCommissionFormValues>({
      resolver: yupResolver(schema),
      defaultValues: schema.getDefault(),
    });

  const toRequest = useCallback(
    (form: LoanCommissionFormValues): LoanCommissionRequestDto => ({
  ...
  const callApi = useCallback(
    (request: LoanCommissionRequestDto, format: string) =>
      loanService.api.loanCommissionCreate(request, { format }),
    [],
  );
  ...
  const onSubmit: SubmitHandler<LoanCommissionFormValues> = useCallback(
