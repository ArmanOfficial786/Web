...
export interface CollectorWiseAccountCloseResponseExtended extends ReportResponseDtos {
  blobUrl: string;
  isLoading: boolean;
}

const schema: yup.ObjectSchema<CollectorWiseAccountCloseFormValues> = yup
  .object({
    fromDateBs: yup
      .string()
...
export default function CollectorWiseAccountClosePage() {
  const [reportState, setReportState] =
    useState<CollectorWiseAccountCloseResponseExtended>({
      blobUrl: "",
      isLoading: false,
    });
  const [lastRequest, setLastRequest] =
    useState<CollectorWiseAccountCloseRequestDto | null>(null);
  const { collectorOptions } = useReportFormContext();

  const { control, handleSubmit, setValue, reset } =
    useForm<CollectorWiseAccountCloseFormValues>({
  ...
  const toRequest = useCallback(
    (
      form: CollectorWiseAccountCloseFormValues,
    ): CollectorWiseAccountCloseRequestDto => {
      const collectorName =
  ...
  const callApi = useCallback(
    (request: CollectorWiseAccountCloseRequestDto, format: string) =>
      memberAccountService.api.collectorWiseAccountCloseCreate(request, {
        format,
      }),
    [],
  );
    ...
    async (format: ReportFormat) => {
      ...
      try {
        ...
        link.download = extractFilenameFromResponse(
          res,
          format,
          "CollectorWiseAccountCloseReport",
        );
  ...
  const onSubmit: SubmitHandler<CollectorWiseAccountCloseFormValues> =
    useCallback(
