...
import { Chip } from "@mui/material";
import type { FieldValues } from "react-hook-form";
import type { EntityLookupConfig, LookupPage } from "../../types/lookup";
import { memberLookUpService } from "@/services/Common/MemberLookUpService";
import type { MemberLookUpDtos } from "types/api/api";
...
export function MemberLookupConfig<
  TForm extends FieldValues,
>(): EntityLookupConfig<MemberRecord, MemberFilterFields, TForm> {
  return {
    cacheKey: "member-lookup",
    title: "Member Directory",
    rowKey: "memMemberRegistrationId",
    filterDefaults: {
      centerName: "",
    ...
    /**
     * memberLookUpService.getAllWithFilters returns MemberLookUpDtosPagination
     * DIRECTLY: { currentPage, totalPages, pageSize, totalRecord,
     * hasNextPage, hasPreviousPage, items }. No .data / .pagination nesting.
     */
    fetchPage: async (page): Promise<LookupPage<MemberRecord>> => {
      const res = await memberLookUpService.getAllWithFilters({ Page: page });
      return {
        items: res.items ?? [],
        totalPages: res.totalPages ?? 1,
        currentPage: res.currentPage ?? page,
        totalRecord: res.totalRecord ?? undefined,
      };
    },
    mapToFormValues: (row) =>
      ({ memberId: row.memberId, memberName: row.memberName }) as any,
